...
class GramaticaBNF:
    def __init__(self):
        ...
        # Patrón para verificar estructura palindrómica necesaria para S → 0S0 | 1S1
        # Captura primer dígito, contenido medio, y último dígito
        self.patron_estructura = re.compile(r'^([01])(.*?)([01])$')
    
    def validar_cadena(self, cadena):
        """
        Valida si una cadena pertenece al lenguaje generado por la gramática
        S → 0S0 | 1S1 | 0 | 1
        """
        cadena = cadena.strip()
        ...
        # Validar que solo contenga dígitos binarios usando regex
    ...
    def _aplicar_reglas_bnf(self, cadena):
        """
        Aplica las reglas de la gramática BNF de forma recursiva
        """
        ...
        # Usar regex para extraer la estructura
        ...
        # Verificar que primer y último dígito sean iguales (regla S → 0S0 o S → 1S1)
        if primer_digito != ultimo_digito:
            return False
        
        # Si el contenido medio está vacío, es válido (equivale a S → 00 | 11)
